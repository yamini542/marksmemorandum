...
<body>
 <div id="wrap">
	...
	<option value="<?php echo $r['year'];?>" > <?php echo $r['year'];?></option>
						...
						$yyy=substr($yy,2,2);
						$y=$yyy+1;
						$query="select * from $sem where (credits>0 and (substr(hallno,1,2)='$yyy' or (substr(hallno,1,2)='$y' and substr(hallno,4,2)='b5'))) group by hallno ";
	$sql=mysqli_query($con,$query);
	$rc1=mysqli_num_rows($sql);
	$query="select * from $sem where (credits<=0 and (substr(hallno,1,2)='$yyy' or (substr(hallno,1,2)='$y' and substr(hallno,4,2)='b5'))) group by hallno ";
	$sql=mysqli_query($con,$query);
	$rc2=mysqli_num_rows($sql);
	echo "pass percentage:";echo ($rc1/($rc1+$rc2))*100;
	echo "</fieldset>";
						}
						?>